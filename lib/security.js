...
const SECURITY_CONFIG = {
  ...
  rateLimiting: {
    ...
    timeWindow: parseInt(process.env.OTP_RATE_LIMIT_TIME_WINDOW) || 5 * 60 * 1000,
    cooldownPeriod: parseInt(process.env.OTP_RATE_LIMIT_COOLDOWN) || 60 * 1000
  },
  
  // Verification rate limiting
  verificationRateLimit: {
    maxAttempts: parseInt(process.env.VERIFICATION_RATE_LIMIT_MAX_ATTEMPTS) || 10,
    timeWindow: parseInt(process.env.VERIFICATION_RATE_LIMIT_TIME_WINDOW) || 15 * 60 * 1000,
    lockoutDuration: parseInt(process.env.VERIFICATION_LOCKOUT_DURATION) || 30 * 60 * 1000
  },
...
function getSecurityHeaders(origin = null) {
  ...
  if (process.env.NODE_ENV === 'production' || SECURITY_CONFIG.securityHeaders.forceHttps) {
  ...
  baseHeaders['Access-Control-Allow-Headers'] = 'Content-Type, X-Requested-With, Authorization';
...
const DEFAULT_BLOCKED_DOMAINS = [
  ...
  '0clickemail.com', '1secmail.com', '20minutemail.com', '2prong.com',
  // Add more as needed
];
