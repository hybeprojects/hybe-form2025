...
// Force using the OS temporary directory to avoid potential sandbox permission issues
const SQLITE_DB_PATH = path.join(os.tmpdir(), 'otp_database.db');

class OTPDatabase {
  constructor() {
    this.driver = process.env.DATABASE_URL || process.env.POSTGRES_URL ? 'pg' : 'sqlite';
  ...
  async initialize() {
    ...
    await new Promise((resolve, reject) => {
      this.db = new sqlite3.Database(SQLITE_DB_PATH, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
        if (err) {
  ...
  async createTables() {
    ...
    await new Promise((resolve, reject) => {
      ...
      // Use db.exec to run all queries sequentially. This prevents race conditions
      // where an index is created before its table exists.
