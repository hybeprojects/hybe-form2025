...
export function getSupabase() {
  if (!supabase) {
    const url = import.meta.env.VITE_SUPABASE_URL || "REPLACE_ENV.SUPABASE_DATABASE_URL";
    const anonKey = import.meta.env.VITE_SUPABASE_ANON_KEY || "REPLACE_ENV.SUPABASE_ANON_KEY";
    if (!url || !anonKey) {
      console.error(
...
export async function verifyEmailOtp(email, token) {
  const client = getSupabase();
  // Use 'signup' for OTPs that use the {{.Token}} variable in the "Confirm signup" email template.
